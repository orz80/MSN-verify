...
export default function App() {
  ...
  const checkFrame = useCallback(async () => {
    ...
    if (!config.apiKey) {
      addLog("API Key missing. Please check settings.", "error");
      stopMonitoring();
      return;
    }
          ...
          <div className="bg-surface rounded-xl p-4 border border-gray-800 shadow-xl space-y-4">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs text-gray-400">
                <span className="flex items-center gap-1"><Key className="w-3 h-3"/> Gemini API Keys</span>
              </div>
              <input 
                type="password"
                placeholder="Paste key(s) here (comma separated)"
                value={config.apiKey}
                onChange={(e) => setConfig(prev => ({ ...prev, apiKey: e.target.value }))}
                className="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1.5 text-xs text-gray-200 focus:outline-none focus:border-primary placeholder-gray-600"
              />
               <p className="text-[10px] text-gray-600">Keys are stored locally in your browser.</p>
            </div>
