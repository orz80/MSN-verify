...
// Parse multiple API keys from the environment variable (comma-separated)
// Example: "key1,key2,key3"
const apiKeys = (process.env.API_KEY || '').split(',').map(k => k.trim()).filter(k => k.length > 0);
let currentKeyIndex = 0;

// Helper to get the next client in rotation
const getNextClient = (): GoogleGenAI | null => {
...
export const checkFrameForTarget = async (
  ...
  if (!ai) {
    console.error("No API Keys found. Please check process.env.API_KEY.");
